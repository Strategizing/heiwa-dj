...
function printStartupBanner(localMode, embeddedEngine, apiPort, localStrudelPort) {
    if (localMode) {
        if (embeddedEngine) {
            console.log('┌─────────────────────────────────────────────────────┐');
...
async function main() {
    ...
    bridge.on('warning', (msg) => {
        state.lastError = msg;
    });
    bridge.on('phrase', (phrase) => {
