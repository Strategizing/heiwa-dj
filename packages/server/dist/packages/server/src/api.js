...
import { enqueueRequest, makeRequest, pushChat } from './state.js';
function serializeBreakerState(breaker) {
...
function buildStatus(state, bridge, breaker) {
    return {
...
function buildStatusEvent(state, bridge, breaker) {
    return {
        type: 'status',
        model: state.activeModel,
        modelMode: state.mode,
...
export function startAPI(opts) {
    const app = express();
    app.use(cors());
    app.use(express.json({ limit: '1mb' }));
    const shouldServeBuiltUi = Boolean(opts.uiDistDir &&
        fs.existsSync(path.resolve(opts.uiDistDir, 'index.html')));
    const builtUiDir = shouldServeBuiltUi ? path.resolve(opts.uiDistDir) : null;
