...
import { z } from 'zod';
import { FALLBACK_GROOVE } from './breaker.js';
import { createDjTools } from './tools.js';
import { dequeueRequest, pushChat, trimHistory, updateTempoDerivedFields } from './state.js';
...
function normalizeAction(raw) {
    ...
    if (action) {
        if (/(transition|xfade|blend|switch|mix)/.test(action))
            return 'transition';
        if (/(hush|stop|silence|mute)/.test(action))
            return 'hush';
        if (/(announce|say|speak|voice)/.test(action))
            return 'announce';
        if (/(done|complete|finished|finish)/.test(action))
            return 'done';
        if (/(play|pattern|build|start|compose|set tempo|setcpm|next move)/.test(action))
            return 'play_pattern';
    }
    if (raw.outgoing && raw.incoming)
        return 'transition';
    if (raw.code)
        return 'play_pattern';
    if (raw.text)
...
function normalizeJsonCommand(raw) {
...
export class DJAgentRunner {
    ...
    async loop() {
        while (this.running) {
            ...
            if (this.state.playbackActive) {
                ...
                try {
                    if (this.breaker.shouldFallbackNow()) {
                        ...
                        if (this.state.patternHistory.length > 100)
                            this.state.patternHistory = this.state.patternHistory.slice(-100);
            ...
            this.state.breakerState = {
                syntax: {
                    state: breakerState.syntax.state,
                ...
                bridge_timeout: {
                    state: breakerState.bridge_timeout.state,
                ...
                model_timeout: {
                    state: breakerState.model_timeout.state,
    ...
    appendTurnToHistory() {
        const systemMessage = { role: 'system', content: buildSystemPrompt(this.state) };
    ...
    async runJsonModeTurn() {
        ...
        if (command.action === 'announce' && command.text) {
            const sayTool = this.tools.announce;
        ...
        if (command.action === 'transition' && command.outgoing && command.incoming) {
            const transitionTool = this.tools.transition;
        ...
        if ((command.action === 'play_pattern' || command.action === 'done') && command.code) {
            const playTool = this.tools.play_pattern;
