...
async function hasToolCapability(model) {
    try {
...
async function singleToolProbe(model) {
    const payload = {
        model,
        stream: false,
...
export async function probeModel(model) {
    ...
    if (cap.hasTools) {
        for (let i = 0; i < attempts; i += 1) {
            if (await singleToolProbe(model))
                ok += 1;
        }
    }
    const mode = cap.hasTools && ok >= 8 ? 'tool' : 'json';
    return {
        model,
        mode,
...
export async function selectActiveModel(models = ['qwen2.5-coder:7b']) {
