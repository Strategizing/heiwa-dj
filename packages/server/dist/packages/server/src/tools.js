...
async function sendPattern(ctx, input) {
    ...
    if (state.patternHistory.length > 100)
        state.patternHistory = state.patternHistory.slice(-100);
...
export function createDjTools(ctx) {
    ...
    return {
        ...
        read_request: tool({
            ...
            execute: async () => {
                ...
                if (!req)
                    return { found: false, message: 'No pending requests' };
                return {
                    found: true,
                    id: req.id,
