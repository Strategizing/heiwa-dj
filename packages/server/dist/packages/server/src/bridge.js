...
export class DJBridge extends EventEmitter {
    ...
    getClientConnectionState() {
        if (this.clientConnected)
            return 'connected';
        if (this.clientConnecting || this.browserClients.size > 0)
            return 'connecting';
        return 'none';
    }
    waitForNextPhrase(lastSeenPhrase) {
